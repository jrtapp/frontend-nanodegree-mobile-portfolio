...
var pizzaElementGenerator = function(i) {
  ...
  // #JT - Removed inline style and col-md-6 class since flexbox seemed to be more efficient. 
...
var resizePizzas = function(size) {
  ...
  */
  ...
  function changePizzaSizes(size) {
    ...
    /* #JT - Moved reflow inducing calls out of loop
       Before: 274.4 ms
       After: 44.6 ms

       #JT - Changed from querySelectorAll to getElementsByClassName
       Before: 44.6 ms
       After: 17.9 ms
    */
...
function updatePositions() {
  ...
  /* #JT Change from querySelectorAll to getElementsByClassName
      Before: < 137 ms
      After: < 30 ms
  ...
  */
  ...
  /* #JT
      Set offset to represent current document scroll position at frame drawing time
      Set phase array to generate lateral offset for pizzas using sin function
  */
  var offset = document.body.scrollTop / 1250;
  var phase = [];
  for(var k = 0; k < 5; k++){phase[k] = Math.sin(offset + k);} 

  /* #JT
      Loop through items.  Outer loop, i, steps through all items stepping by 5, 
      inner loop, j, steps from 0-4 to replicate modulus with having to do math.
      Sum of i and j represents each moveable pizza.  Pizza left position is its
      current left starting position plus 100 times the phase value based on its
      position in the order.
  */
  for (var i = 0; i < items.length; i += 5) {
    for(var j = 0; j < 5; j++)
    {
      items[i + j].style.left = items[i + j].basicLeft + 100 * (phase[j]) + 'px';
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var s = 256;
  /*
   #JT - Dropped moving pizza count to 40 from 200
    Page Load Before: 1400 ms
    Page Load After: 700 ms

    #JT - Moved width and height style settings to CSS
  */
  for (var i = 0; i < 40; i++) {
